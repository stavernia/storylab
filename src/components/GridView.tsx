...
export function GridView({
  ...
  const [selectedChapterForContent, setSelectedChapterForContent] =
  ...
  const loadCellTags = async (chapterId: string, themeId: string) => {
    ...
    try {
      ...
      console.error(
        `Failed to load tags for cell ${chapterId}:${themeId}:`,
        error,
      );
    ...
    openInspector({
      ...
      content: (
        <GridCellInfoForm
          ...
          threadRole={
            (getCell(chapterId, themeId)?.threadRole as ThreadRole | null) ??
            undefined
          }
          tags={cellTags}
  ...
  useEffect(() => {
    if (selectedTheme) {
      ...
      if (
        updatedTheme &&
        JSON.stringify(updatedTheme) !== JSON.stringify(selectedTheme)
      ) {
  ...
  useEffect(() => {
    if (selectedGridCell) {
      ...
      setCellNote(
        getNote(selectedGridCell.chapterId, selectedGridCell.themeId),
      );
    } else {
      setCellTags([]);
    }
  ...
  const getCellData = (chapterId: string, themeId: string): GridCell => {
    ...
    return {
      ...
      intensity:
        typeof existing?.intensity === "number" ? existing.intensity : 0,
  ...
  const sortedThemes = [...themes].sort((a, b) => {
    const aOrder =
      typeof a.rowOrder === "number" ? a.rowOrder : Number.MAX_SAFE_INTEGER;
    const bOrder =
      typeof b.rowOrder === "number" ? b.rowOrder : Number.MAX_SAFE_INTEGER;
  ...
  if (parts && parts.length > 0) {
    ...
    if (unassignedChapters.length > 0) {
      const firstIndex = chapters.findIndex(
        (ch) => ch.id === unassignedChapters[0].id,
      );
      partSpans.push({
        part: null,
        startIndex: firstIndex,
        count: unassignedChapters.length,
      });
    }
  }
  ...
  return (
    <div className="flex h-full flex-col">
                ...
                <thead>
                  ...
                  {partSpans.length > 0 && (
                    <tr className="sticky top-0 z-20">
                      <th
                        className={`sticky left-0 z-30 bg-gray-50 border-b border-r border-gray-200 p-2 text-left ${
                          columnSize === "full"
                            ? "min-w-[140px] max-w-[180px]"
                            : "min-w-[75px] max-w-[95px]"
                        }`}
                      >
                  ...
                  <tr className="sticky top-[41px] z-20">
                    <th
                      className={`sticky left-0 z-30 bg-gray-100 border-b border-r border-gray-200 p-3 text-left ${
                        columnSize === "full"
                          ? "min-w-[140px] max-w-[180px]"
                          : "min-w-[75px] max-w-[95px]"
                      }`}
                  ...
                  return (
                    <tbody key={group.id}>
                      ...
                      <tr className="sticky left-0 z-10">
                        <td className="sticky left-0 z-10 bg-gray-50 border-b border-gray-200 px-3 py-2 min-w-[110px] max-w-[140px]">
      ...
      {/* Add Theme Dialog */}
      <Dialog
        open={showAddThemeDialog ?? isAddingTheme}
        onOpenChange={setShowAddThemeDialog ?? setIsAddingTheme}
      >
      ...
      {/* Add Chapter Dialog */}
      <Dialog
        open={showAddChapterDialog ?? isAddingChapter}
        onOpenChange={setShowAddChapterDialog ?? setIsAddingChapter}
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button
              ...
              variant="outline"
              onClick={() =>
                (setShowAddChapterDialog ?? setIsAddingChapter)(false)
              }
            >
              Cancel
            </Button>
      ...
      {editingChapterId === chapter.id ? (
        ...
        <>
          ...
          {size === "minimal" ? (
            ...
            <>
              <div
                className={`${size === "compact" ? "text-xs" : "text-sm"} text-gray-900 mb-1 pr-6`}
              >
                {chapter.title}
              </div>
  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        const threadBounds =
          rowMode === "thread"
            ? getThreadBounds(theme, chapters, (chapterId) =>
                getCell(chapterId, theme.id),
              )
              ...
              {columnSize === "minimal" ? (
                <div className="relative flex items-center justify-center p-2">
                  {rowMode === "presence" ? (
                    ...
                    (() => {
                      ...
                      const role: ThreadRole =
                        (note?.threadRole as ThreadRole) ?? "none";
                ...
                <div className="relative p-3">
                  ...
                  <div className="flex items-center justify-between mb-2">
                      ...
                      (() => {
                        ...
                        if (threadBounds.eventIndex >= 0) {
                          const distance = Math.abs(
                            colIndex - threadBounds.eventIndex,
                          );
                          if (distance === 0) lineThicknessClass = "h-[3px]";
                          else if (distance === 1)
                            lineThicknessClass = "h-[2.5px]";
                        }
                    ...
                    <button
                      onClick={() =>
                        handleOpenCellInspector(chapter.id, theme.id)
                      }
