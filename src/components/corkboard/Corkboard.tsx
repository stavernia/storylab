...
import { Search, Plus, LayoutGrid, LayoutList, Layers, PanelLeftClose, PanelLeftOpen, BookOpen, FolderOpen, ChevronLeft, ChevronRight } from 'lucide-react';
import {
  CorkboardCard,
  corkboardApi,
  CorkboardBoard,
...
interface CorkboardProps {
  ...
  // NEW: Selection state from parent (like EditorView and OutlineView)
  selection: ManuscriptSelection;
  onSelectionChange?: (selection: ManuscriptSelection) => void;
}

// Selection types (matching EditorView)
...
export function Corkboard({
  ...
  // Selection handlers (matching EditorView)
  ...
  const computeVisibleRows = (): CorkboardRowType[] => {
    ...
    if (effectiveSelection.kind === 'manuscript') {
      ...
      const chapter = sortedChapters.find(ch => ch.id === effectiveSelection.chapterId);
      if (chapter) {
  ...
  const handleCreateCard = async (
    ...
    const rowCards = cards.filter(c => {
      if (scope === 'book') {
        ...
        return c.scope === 'part' && c.partId === partId && c.boardId === currentBoardId;
      } else {
        return c.scope === 'chapter' && c.chapterId === chapterId && c.boardId === currentBoardId;
      }
    }).sort((a, b) => a.laneRank.localeCompare(b.laneRank));
    
    const newRank = rowCards.length > 0
  ...
  const handleCardDrop = async (
    ...
    const targetRowCards = cards
      .filter(c => {
        ...
        const matchesBoard = c.boardId === currentBoardId || (!c.boardId && currentBoardId === 'main-board');
    ...
    const newRank = targetRowCards.length > 0
      ? getBetweenRank(targetRowCards[targetRowCards.length - 1].laneRank, undefined)
      : getInitialRank();
  ...
  const handleCardReorder = async (
    ...
    const cardInSameRow = 
      ...
      (targetRowType === 'part' && card.scope === 'part' && card.partId === targetPartId) ||
      (targetRowType === 'chapter' && card.scope === 'chapter' && card.chapterId === targetChapterId);
    ...
    if (targetIndex === -1) {
      ...
      console.error('Target card details:', cards.find(c => c.id === targetCardId));
      return;
    }
    ...
    if (cardInSameRow) {
      ...
      const targetIndexWithoutDragged = cardsWithoutDragged.findIndex(c => c.id === targetCardId);
      
      if (position === 'before') {
        // Insert before target card
        const prevCard = targetIndexWithoutDragged > 0 ? cardsWithoutDragged[targetIndexWithoutDragged - 1] : null;
        ...
        const nextCard = targetIndexWithoutDragged < cardsWithoutDragged.length - 1 ? cardsWithoutDragged[targetIndexWithoutDragged + 1] : null;
        newRank = getBetweenRank(prevCard.laneRank, nextCard?.laneRank);
      }
    } else {
      ...
      if (position === 'before') {
        // Insert before target card
        const prevCard = targetIndex > 0 ? targetRowCards[targetIndex - 1] : null;
        const nextCard = targetRowCards[targetIndex];
        newRank = getBetweenRank(prevCard?.laneRank, nextCard.laneRank);
      } else {
        // Insert after target card (default behavior)
        const prevCard = targetRowCards[targetIndex];
        const nextCard = targetIndex < targetRowCards.length - 1 ? targetRowCards[targetIndex + 1] : null;
        newRank = getBetweenRank(prevCard.laneRank, nextCard?.laneRank);
      }
    }
...
function CorkboardContent({
  ...
  handleCreateCard: (scope: 'book' | 'part' | 'chapter', partId: string | null, chapterId: string | null) => void;
      ...
      {visibleRows.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-center">
          <div className="text-gray-400 mb-4">
            ...
            <p className="text-sm">Create chapters or parts to organize your cards</p>
          </div>
        </div>
